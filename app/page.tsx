...
export default function Home() {
    ...
    const onClick = useCallback((year: number) => {
        const findIssues = allIssue.find(issue => issue.year == year);
        if(!findIssues && year != 0) toast.error(`${year}년에는 현재 데이터가 없습니다.`);
        setIssue(findIssues)
    }, [])

    const onEnter = useCallback((e: any) => {
        if(e.keyCode == 13){
            const findIssues = allIssue.find(issue => issue.year == e.target.value);
            if(!findIssues) toast.error(`${e.target.value}년에는 현재 데이터가 없습니다.`);
            setIssue(findIssues);
        }
    }, [])
                ...
                {issue.history.map((history, index) =>
                    ...
                    <div className={'mb-5'}>
                        ...
                        <div>
                            ...
                            <h1 className="text-3xl text-red-950 font-bold">Kolosseum입니다.</h1>
                        </div>
