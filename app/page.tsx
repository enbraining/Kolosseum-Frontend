  ...
  return (
    <main className="grid w-full">
        <div className="absolute w-full overflow-x-scroll scrollbar-hide">
            ...
            {years.map(value =>
                <div onClick={() => {setYear(findYear(value))}} key={value} className={`${value == year?.year ? "text-4xl text-stone-700 font-black" : "text-2xl"} font-bold hover:font-black hover:text-4xl hover:text-stone-700 duration-150`}>{value}</div>)
            }
            </div>
        </div>
